  ...
  <el-card>
    ...
    <el-form ref="myForm" :model="formData" :rules="rules" label-width="80px" class="publish-form">
      ...
      <el-form-item prop='content' label="内容">
        <quill-editor style='width:800px;height:400px;margin-bottom:100px' v-model="formData.content" type="textarea" :rows="4" placeholde="请输入内容"></quill-editor>
      </el-form-item>
...
export default {
  ...
  methods: {
    changeType () {
      ...
      if (this.formData.cover.type === 1) {
        ...
        this.formData.cover.images = ['', '', '']// 当类型为3图时这时要为数组生成3个地址
      } else {
  ...
  created () {
    let { articleId } = this.$route.params// 加载数据 如果id存在 就认为是编辑 如果不存在就认为是新增
